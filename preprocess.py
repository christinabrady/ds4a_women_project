...
def gen_categories_hotels(yelp_business):
    ...
    include_categ_hotels = yelp_business['categories'].str.contains(
        'Hotels', na=False)
...
def get_restaurants_usa(yelp_business, category_tagged):
    """Extract restaurant business located in the U.S.
       ...
       yelp_business: a pd dataframe of yelp business dataset
       category_tagged: a string, the name of a csv file that contains 
       a list of categories and an indicator whether the category 
       will be excluded
       return a dataframe of restaurants in the U.S.
    """

    # identify businesses that tag themselves in 'Restaurant' category
    include_categ_restaurant = yelp_business['categories'].str.contains(
    ...
    category_excluded = list(
        category_df[category_df['exclude'] == 1]['category'])
    ...
    condition = restaurant_raw_df.categories.apply(
        check_categories, args=(category_excluded,))
    ...
    restaurants_df = restaurant_clean_by_categories.drop(
        'not_restaurant', axis=1)

    # non US states in Yelp business dataset
    non_us_states = ["ABE", "BC"]

    # extract US restaurants
    restaurants_us = restaurants_df[~restaurants_df['state'].isin(
        non_us_states)]
...
def get_top_cities(yelp_restaurants, n=5):
    ...
    top_n = counted_by_city.sort_values(
        'number_of_restaurants', ascending=False).head(5)['city'].str.upper().tolist()
...
def get_ids_reviews_pre_covid(yelp_reviews):
    ...
    yelp_reviews['date'] = pd.to_datetime(
        yelp_reviews['date'], format='%Y-%m-%d')
    ...
    ids_reviews_pre_covid = filtered_df.drop_duplicates(subset='business_id')[
        ['business_id']]
...
def clean_states(restaurant_df, n=5):
    ...
    states_all_sorted = restaurant_df.groupby("state", as_index=False).agg(
        {'business_id': 'count'}).sort_values(by="business_id", ascending=False)
...
def clean_city_names(restaurant_df, cutoff=85):
    ...
    for city_name in city_list:
        matches = process.extract(
            city_name, restaurant_df['city'], limit=len(restaurant_df.city))
        for match in matches:
            if match[1] >= cutoff:
                restaurant_df.loc[restaurant_df['city']
                                  == match[0], 'city'] = city_name
...
def gen_category_column(restaurant_df, mapping_csv, category_type):
    ...
    condition = restaurant_df.categories.apply(
        check_categories, args=(category_list,))

    # map the category
    category_mapped = restaurant_df.categories.apply(
        map_category_type, args=(category_list, category_dict))
